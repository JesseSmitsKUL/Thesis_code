...
def singleRun():
    ...
    for key in datakeys:
        ...
        model = SeSStream("khronos", set)
        model.runSimulation()

        break

def evaluateRuns():
    rootFolder = './Stat_Evaluation/'

    print(rootFolder)

    csv_files = {'inc': "../Datasets/rats_inc.csv", 'static': "../Datasets/rats_static.csv"}

    models = ['AR',"MA", "SES", "HOLT"]
    ...
    # dd/mm/YY H:M:S
    dt_string = now.strftime("%d-%m-%Y_%H-%M-%S_cutoff_07/")
    ...
    for file in csv_files.keys():
        ...
        for key in datakeys:
            ...
            for modelname in models:
                ...
                if modelname == 'AR':
                    model = ARStream("Ar", set[cutoff:])
                elif modelname == 'MA':
                    model = MAStream("Ma", set[cutoff:])
                elif modelname == 'SES':
                    model = SeSStream("Ses", set[cutoff:])
                elif modelname == 'HOLT':
                    model = HoltStream("Holt", set[cutoff:])
                else:
