...
class ARMAXKhronos_K_es(BaseStream):

    def __init__(self, name, times, window, loc):
        super(ARMAXKhronos_K_es, self).__init__(name, times)
        self.alpha = 7.0 / 8.0
        self.beta = 3.0 / 4.0
        self.variance = -1
        ...
        self.processOutofOrder = 0
        self.sname = ''

        self.comp_window_size = 600
        self.variances = []

        self.indexToEval = (2*self.comp_window_size) + 2
    ...
    def update(self):
        ...
        # self.prediction = self.predictionAR
        #
        # minVal = 0
        # maxVal = 0
    ...
    def updateKVal(self):
        ...
        for k in k_values:
            ...
            if keysValidated(self.constrains, newConstraints):
                for key in self.constrains:
                    # 0.4 sufficient for constraints above 0.5
                    # key is more robuust especially for inc set
    ...
    def increment(self, rat):
        if len(self.variances) >= (2*self.comp_window_size) and len(self.variances) % 200 == 0:
            self.updateKVal()
    ...
    def updateArrival(self):
        if self.index == -1:
            ...
            # if len(self.variances) == (12 * self.comp_window_size):
            #     return
    ...
    def runSimulation(self):
        ...
        if self.filewriter != None:
            ...
            self.filewriter.write("\nTrendAcc: " + str(self.trendAccuracy) + '\t\t NNACC: ' + str(self.NNTrendAcc) +
        ...
        print("TrendAcc: ", self.trendAccuracy, '\t\t NNACC: ', self.NNTrendAcc,
    ...
    def visualizeData(self):
        ...
        for key in self.constrains:
            ...
            for x in range(25):
                ...
                plt.plot(self.timeouts[key][(x - 1) * datapoints:  x * datapoints], label='prediction')
                plt.plot(self.results[(x - 1) * datapoints:  x * datapoints], label='res')
                plt.legend()
            ...
            plt.plot(self.predictedArrivals, label='arrivalall')
            plt.plot(self.timeouts[key], label='predictionall')
